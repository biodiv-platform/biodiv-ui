...
const SortableCustomFieldList = ({ userGroupId, customFieldList, setCustomFieldList }) => {
  const [reorder, setReorder] = useState<boolean>();
  const { t } = useTranslation();

  const onSortEnd = ({ oldIndex, newIndex }) => {
    setCustomFieldList(arrayMove(customFieldList, oldIndex, newIndex));
    setReorder(true);
  };

  const handleReorderCustomField = async () => {
    const payload = customFieldList.map((field, index) => ({
  ...
  const removeCustomField = async (index) => {
    ...
    if (success) {
      setCustomFieldList(customFieldList.filter((item) => item.customFields.id !== index));
  ...
  useEffect(
    () => setCustomFieldList(customFieldList.sort((a, b) => a.displayOrder - b.displayOrder)),
    []
  );
  return (
    <CustomFieldList
