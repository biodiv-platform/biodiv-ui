...
export default function UserTemporalObservedOn(userId) {
    ...
    const handleDownload = async() => {
      ...
      axAddDownloadLog("User",window.location.href,"Temporal Distribution - Month Observed")
    };
  ...
  return (
      <Box className="white-box" mb={4}>
        <BoxHeading styles={{display:"flex",justifyContent:"space-between"}}>📊 {t("user:observations.temporal_month_observed")} <Button onClick={handleDownload} variant="ghost" colorScheme="blue"><DownloadIcon/></Button></BoxHeading>
      <Box p={4}>
