...
import { MENU_PORTAL_TARGET, RESOURCE_SIZE, UNSUPPORTED_FILE_TYPES } from "@static/constants";
...
export default function ResourceCard({ resource, index }: IResourceCardProps) {
  ...
  if (UNSUPPORTED_FILE_TYPES.includes(resource.type)) {
    removeObservationAsset(resource.hashKey);
    return <></>;
  } else {
    return (
      <Flex
